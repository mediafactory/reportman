...
type
 ...
 TRpPreviewMeta=class(TScrollBox)
  ...
  published
   ...
   property Color;
   property OnWorkProgress:TMetaFileWorkProgress read FOnWorkProgress write FOnWorkProgress;
 end;
...
begin
 ...
 begin
  ...
  if (FEntirePageCount>1) then
  begin
   metafile.RequestPage(FPage+FEntirePageCount-1);
...
begin
 ...
 begin
  FMetafile.RequestPage(FPage+EntirePageCount-1);
  if (FPage>=FMetafile.CurrentPageCount) then
 ...
 intentirepcount:=1;
 GetPhysicSize;
 clients.Width:=Parent.ClientWidth-FBarWidth;
 clients.Height:=Parent.ClientHeight-FBarHeight;
...
begin
 ...
 posy:=Trunc(offsety-(Parent.ClientHeight-Fbarheight)/2);
 posx:=Trunc(offsetx-(Parent.ClientWidth-Fbarwidth)/2);
