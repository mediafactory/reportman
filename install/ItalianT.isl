...
; *** SetupLdr messages
SetupLdrStartupMessage=%1 sarà installato, desideri continuare?
...
UninstallAppRunningError=La disinstallazione ha rilevato che %1 è in esecuzione.%n%nChiudere tutte le installazioni precedenti e premere OK per continuare oppure Cancella per uscire.

; *** Misc. errors
ErrorCreatingDir=L'installazione non ha potuto creare la cartella "%1".
ErrorTooManyFilesInDir=Non è possibile creare ulteriori files nella cartella "%1" perché ne contiene troppi.

; *** Setup common messages
ExitSetupTitle=Esci dall'installazione
ExitSetupMessage=L'installazione non è completa, se viene interrotta il programma non sarà installato.%n%nPuoi riprovare in un secondo momento. Vuoi uscire?
...
WelcomeLabel2=Verrà ora installato [name/ver].%n%nSi raccomanda di chiudere tutte le applicazioni prima di continuare.
...
SelectDirLabel3=L'installazione copierà [name] nella seguente cartella.
...
DirNameTooLong=Il nome della cartella è troppo lungo.
...
SelectStartMenuFolderLabel3=L'installazione creerà i collegamenti del programma nella seguente cartella del menu Start.
...
GroupNameTooLong=Il nome della cartella è troppo lungo.
...
ReadyLabel1=L'installazione è pronta per copiare [name] sul tuo computer.
...
PreviousInstallNotCompleted=L'installazione/disinstallazione di un programma precedente non è stata completata. Riavviare il computer per terminare l'installazione, quindi procedere nuovamente con l'installazione.
CannotContinue=L'installazione non può continuare. Cliccare su Annulla per terminare.
...
FinishedLabelNoIcons=L'installazione di [name] è stata eseguita con successo.
FinishedLabel=[name] è stato installato con successo: l'applicazione può essere avviata utilizzando le icone presenti nel menu Start.
ClickFinish=Cliccare su Fine per uscire dall'installazione.
FinishedRestartLabel=Per completare l'installazione di [name] è necessario riavviare il computer. Procedere ora?
FinishedRestartMessage=Per completare l'installazione di [name] è necessario riavviare il computer.%n%nProcedere ora?
ShowReadmeCheck=Si, desidero visualizzare le informazioni.
YesRadio=&Si, riavvia il computer.
NoRadio=&No, riavvierò manualmente il computer.
...
ExistingFileReadOnly=Il file è di sola lettura.%n%nRimuovere l'attributo di sola lettura e quindi cliccare su Riprova per provare nuovamente, su Ignora per saltare questo file, o su Cancella per annullare l'installazione.
ErrorReadingExistingDest=Si è verificato un errore leggendo il file:
FileExists=Il file esiste già.%n%nVuoi sovrascriverlo?
ExistingFileNewer=Il file esistente è più recente di quello da installare, sì raccomanda di conservarlo.%n%nVuoi conservarlo?
ErrorChangingAttr=Si è verificato un errore nel tentativo di modifica degli attributi del file:
ErrorCreatingTemp=Si è verificato un errore durante la creazione di un file nella cartella di destinazione:
ErrorReadingSource=Si è verificato un errore durante la lettura del file file sorgente:
ErrorCopying=Si è verificato un errore nella copia del file:
ErrorReplacingExistingFile=Si è verificato un errore nella sovrascrittura del file:
ErrorRestartReplace=RiavviaRimpiazza fallito:
ErrorRenamingTemp=Si è verificato un errore rinominando un file nella cartella di destinazione:
...
ErrorOpeningReadme=Si è verificato un errore nell'apertura del file LEGGIMI.
ErrorRestartingComputer=L'installazione non è riuscita a riavviare il computer, procedere manualmente.
...
ConfirmDeleteSharedFile2=Il sistema indica che il file condiviso non è più usato da alcun programma. Vuoi disinstallarlo?%n%nSe non si è sicuri, lasciare il file sul computer, non produrrà alcun danno.
